...
public class AjouterEvenementController implements Initializable {
    ...
    @FXML
    private JFXTextField id_evenement_fx;

    @FXML
    private JFXTextField nom_evenement_fx;

    @FXML
    private JFXTextField lieu_evenement_fx;

    @FXML
    private JFXTextField description_evenement_fx;

    @FXML
    private JFXTextField capacite_evenement_fx;
    @FXML
    ...
    @FXML
    private StackPane ajouterEvenementStackPane;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // design hbox
        /* try {
            JFXDepthManager.setDepth(info_evenement_fx, 1);
    ...
    @FXML
    void AjouterEvenement(ActionEvent event) throws SQLException, FileNotFoundException, IOException {
        ...
        /*
		 * try { File image = new File(afficheEvenement); FileInputStream fis = new
		 * FileInputStream(image); ByteArrayOutputStream bos = new
		 * ByteArrayOutputStream(); byte[] buf = new byte[1024]; for (int readNum;
		 * (readNum = fis.read(buf)) != -1;) { bos.write(buf, 0, readNum);
		 * 
		 * } photo = bos.toByteArray(); fis.close(); bos.close(); } catch (Exception e)
        ...
        JFXDialog dialog = new JFXDialog(ajouterEvenementStackPane, dialogLayout, JFXDialog.DialogTransition.TOP);
        ...
        evenement.setId_user(recupererUtilisateurConnecte.getId_Utilisateur());

        int status = ServiceEvenement.ajouterEvenement(evenement);

        if (status > 0) {
            Node graphic = null;
            ServiceEvenement.notification("Notification d'ajout", Pos.BOTTOM_RIGHT, graphic, "Evenement Ajouter");
            ServiceEvenement.notificationBuilder.show();

            FXMLLoader loader = new FXMLLoader(getClass().getResource("/projet/presentation/AccueilEvenementGUI.fxml"));
            Parent root = loader.load();
            EvenementController controller = (EvenementController) loader.getController();
            controller.setDataAccueil();

        } else {
            Node graphic = null;
            ServiceEvenement.notification("Notification d'ajout", Pos.BOTTOM_RIGHT, graphic, "Erreur d'ajout");
            ServiceEvenement.notificationBuilder.show();
            return;
        }
    ...
    private String replaceFile(String file) {
        ...
        Path targetDirectory = Paths.get("C:\\xampp\\htdocs\\worldfriendship\\web\\Evenement\\image\\affiches\\" + filename);
